...
export function VenueStateOverlay({ songId }: { songId: string }): React.JSX.Element {
  ...
  return (
    <div className="venue-state-overlay">
      <div className="venue-state-title">{t('uiOverlays.venuePreview.title')}</div>
      <div className="venue-state-row"><span>{t('uiOverlays.venuePreview.lighting')}</span><strong>{playbackState.lighting ? formatVenueLabel(playbackState.lighting.type) : t('uiOverlays.venuePreview.none')}</strong></div>
      <div className="venue-state-row"><span>{t('uiOverlays.venuePreview.postFx')}</span><strong>{playbackState.postProcessing ? formatVenueLabel(playbackState.postProcessing.type) : t('uiOverlays.venuePreview.none')}</strong></div>
      <div className="venue-state-row"><span>{t('uiOverlays.venuePreview.camera')}</span><strong>{playbackState.cameraCut ? formatVenueLabel(playbackState.cameraCut.subject) : t('uiOverlays.venuePreview.none')}</strong></div>
      <div className="venue-state-row"><span>{t('uiOverlays.venuePreview.stage')}</span><strong>{playbackState.activeStage.length > 0 ? playbackState.activeStage.map((event) => formatVenueLabel(event.effect)).join(', ') : t('uiOverlays.venuePreview.none')}</strong></div>
      <div className="venue-state-row"><span>{t('uiOverlays.venuePreview.performer')}</span><strong>{playbackState.activePerformer.length > 0 ? playbackState.activePerformer.map((event) => `${formatVenueLabel(event.type)} ${formatVenueLabel(event.performer ?? '')}`.trim()).join(', ') : t('uiOverlays.venuePreview.none')}</strong></div>
      <div className="venue-state-row"><span>{t('uiOverlays.venuePreview.nextEvent')}</span><strong>{playbackState.nextEventTick !== null ? `t${playbackState.nextEventTick}` : t('uiOverlays.venuePreview.end')}</strong></div>
    </div>
  )
}
...
export function InstrumentToggles(): React.JSX.Element {
  ...
  const instruments: { id: Instrument; label: string }[] = [
    { id: 'drums', label: t('uiOverlays.instruments.drums') },
    { id: 'guitar', label: t('uiOverlays.instruments.guitar') },
    { id: 'bass', label: t('uiOverlays.instruments.bass') },
    { id: 'vocals', label: t('uiOverlays.instruments.vocals') },
    { id: 'keys', label: t('uiOverlays.instruments.keys') },
    { id: 'proKeys', label: t('uiOverlays.instruments.proKeys') },
    { id: 'proGuitar', label: t('uiOverlays.instruments.proGuitar') },
    { id: 'proBass', label: t('uiOverlays.instruments.proBass') }
  ]
  ...
  return (
    <div className="instrument-toggles">
      ...
      <button
        ...
        title={t('uiOverlays.instruments.waveformTitle')}
        onClick={toggleHighwayWaveform}
      >
        {t('uiOverlays.instruments.waveform')}
      </button>
      <select
        className="waveform-source-select"
        title={t('uiOverlays.instruments.waveformSourceTitle')}
      ...
      >
        <option value="__mix__">{t('uiOverlays.instruments.waveformMix')}</option>
        {waveformSources.map((source) => (
          <option key={source.filePath} value={source.filePath}>
            {t('uiOverlays.instruments.waveformSource', { filename: source.filename })}
          </option>
        ))}
      </select>
    </div>
...
export function SnapSelector(): React.JSX.Element {
  ...
  return (
    <div className="preview-snap-selector">
      <label>{t('uiOverlays.snap.label')}</label>
      <select
        value={snapDivision}
        onChange={(e) => {
      ...
      >
        {SNAP_DIVISIONS.map((div) => (
          <option key={div} value={div}>
            {t('uiOverlays.snap.division', { div })}
          </option>
        ))}
      </select>
    </div>
...
export function VocalTrackOverlay({ songId }: { songId: string }): React.JSX.Element {
  ...
  const partLabels = [t('uiOverlays.vocalTrack.parts.main'), 'H1', 'H2', 'H3']
  ...
  return (
    <div className="vocal-track-overlay" style={{ height: 160, userSelect: 'none' }}>
      <div className="vocal-track-label" style={{ flexDirection: 'column', gap: 6 }}>
        <span>🎤 {t('uiOverlays.vocalTrack.label')}</span>
      ...
      {open && panelStyle && (
        ...
        >
          ...
          <div className="shortcut-help-section">
            ...
            <ShortcutRow keys={hotkeys.toolSelect} desc={t('uiOverlays.shortcutHelp.desc.selectTool')} />
            <ShortcutRow keys={hotkeys.toolPlace} desc={t('uiOverlays.shortcutHelp.desc.placeTool')} />
            <ShortcutRow keys={hotkeys.toolErase} desc={t('uiOverlays.shortcutHelp.desc.eraseTool')} />
          </div>
          <div className="shortcut-help-section">
            <div className="shortcut-help-section-title">{t('uiOverlays.shortcutHelp.sections.noteModifiers')}</div>
            <ShortcutRow keys={hotkeys.toggleCymbalOrTap} desc={t('uiOverlays.shortcutHelp.desc.cymbalTap')} />
            <ShortcutRow keys={hotkeys.toggleGhostOrHopo} desc={t('uiOverlays.shortcutHelp.desc.ghostHopo')} />
            <ShortcutRow keys={hotkeys.toggleAccent} desc={t('uiOverlays.shortcutHelp.desc.accent')} />
            <ShortcutRow keys={hotkeys.toggleOpenOrKick} desc={t('uiOverlays.shortcutHelp.desc.openKick')} />
            <ShortcutRow keys={hotkeys.toggleStarPower} desc={t('uiOverlays.shortcutHelp.desc.starPowerMode')} />
            <ShortcutRow keys={hotkeys.toggleSolo} desc={t('uiOverlays.shortcutHelp.desc.soloMode')} />
          </div>
          <div className="shortcut-help-section">
            <div className="shortcut-help-section-title">{t('uiOverlays.shortcutHelp.sections.editing')}</div>
            <ShortcutRow keys={hotkeys.copy} desc={t('uiOverlays.shortcutHelp.desc.copy')} />
            <ShortcutRow keys={hotkeys.paste} desc={t('uiOverlays.shortcutHelp.desc.paste')} />
            <ShortcutRow keys={hotkeys.undo} desc={t('uiOverlays.shortcutHelp.desc.undo')} />
            <ShortcutRow keys={hotkeys.redo} desc={t('uiOverlays.shortcutHelp.desc.redo')} />
            <ShortcutRow keys={hotkeys.selectAll} desc={t('uiOverlays.shortcutHelp.desc.selectAll')} />
            <ShortcutRow keys={hotkeys.deleteSelection} desc={t('uiOverlays.shortcutHelp.desc.deleteSelected')} />
            <ShortcutRow keys={hotkeys.createStarPower} desc={t('uiOverlays.shortcutHelp.desc.starPowerFromSelection')} />
            <ShortcutRow keys={hotkeys.createSolo} desc={t('uiOverlays.shortcutHelp.desc.soloFromSelection')} />
            <ShortcutRow keys="Esc" desc={t('uiOverlays.shortcutHelp.desc.clearSelection')} />
          </div>
          <div className="shortcut-help-section">
            <div className="shortcut-help-section-title">{t('uiOverlays.shortcutHelp.sections.playbackView')}</div>
            <ShortcutRow keys={hotkeys.playPause} desc={t('uiOverlays.shortcutHelp.desc.playPause')} />
            <ShortcutRow keys={hotkeys.zoomIn} desc={t('uiOverlays.shortcutHelp.desc.zoomIn')} />
            <ShortcutRow keys={hotkeys.zoomOut} desc={t('uiOverlays.shortcutHelp.desc.zoomOut')} />
            <ShortcutRow keys={hotkeys.resetZoom} desc={t('uiOverlays.shortcutHelp.desc.resetZoom')} />
            <ShortcutRow keys={hotkeys.save} desc={t('common.save')} />
          </div>
          <div className="shortcut-help-section">
            <div className="shortcut-help-section-title">{t('uiOverlays.shortcutHelp.sections.clickModifiers')}</div>
            <ShortcutRow keys="Ctrl+Click" desc={t('uiOverlays.shortcutHelp.desc.multiSelect')} />
            <ShortcutRow keys="Click+Drag" desc={t('uiOverlays.shortcutHelp.desc.boxSelect')} />
          </div>
          <div className="shortcut-help-section">
            <div className="shortcut-help-section-title">{t('uiOverlays.shortcutHelp.sections.proGuitarBass')}</div>
            <ShortcutRow keys="Dbl-click" desc={t('uiOverlays.shortcutHelp.desc.editFretInline')} />
            <ShortcutRow keys="↑ / ↓" desc={t('uiOverlays.shortcutHelp.desc.fretAdjust')} />
          </div>
        </div>
      )}
    </div>
